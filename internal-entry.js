...
const { mountInternalApp, APP_VERSION, readStore, writeStore } = require("./internal-app");

const originalFetch = global.fetch;
const crmUrl = String(process.env.CRM_URL || "https://script.google.com/macros/s/AKfycbwAFBxeROd2ZYGJ_h0O7_H2MMxptOMoj3EXIErZpbKuTYFOzOVwQkrk8X1MoxapkHVGSA/exec");
...
if (typeof originalFetch === "function") {
  global.fetch = async function xianjiaweiFetch(input, init = {}) {
    ...
    try {
      ...
      if (url === crmUrl && String(init.method || "GET").toUpperCase() === "POST" && response.ok && init.body) {
        const payload = JSON.parse(String(init.body));
        const result = await response.clone().json().catch(() => ({}));
        const store = readStore();
        const externalId = String(result.orderId || result.order_id || "");
        const duplicate = store.orders.some((item) =>
          (externalId && item.externalId === externalId) ||
          (payload.createdAt && item.sourceCreatedAt === payload.createdAt && item.lineUserId === payload.userId)
        );
        if (!duplicate) {
          const items = (payload.cart || []).map((item) => `${item.name || item.displayName || item.productId || "商品"} × ${item.qty || item.quantity || 1}`).join("\n");
          store.orders.push({
            ...
            status: "新訂單",
            note: [payload.payment, payload.shipping, payload.address].filter(Boolean).join("｜"),
            createdAt: payload.createdAt || new Date().toISOString(),
            updatedAt: new Date().toISOString(),
          });
          if (payload.phone && !store.customers.some((item) => item.phone === payload.phone)) {
          ...
          store.activities.push({
            ...
            actor: "LINE OA",
            action: "自動匯入訂單",
            detail: `${payload.name || "LINE 客戶"}｜${externalId || "未提供訂單編號"}`,
