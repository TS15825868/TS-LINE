...
const CARDS = {
  "hot-water": {
    eyebrow: "小老闆知識卡｜沖泡篇",
    title: ["沖泡一定要用", "滾水嗎？"],
    bullets: ["熱水能均勻化開即可", "不必持續煮滾", "濃淡再依口味調整"],
    mascot: "guide",
  },
  "cold-texture": {
    eyebrow: "小老闆知識卡｜取用篇",
    title: ["冷藏後變得較稠", "怎麼取用？"],
    bullets: ["使用乾燥湯匙", "先取需要的份量", "再用溫熱水慢慢化開"],
    mascot: "guide",
  },
  "sediment": {
    eyebrow: "小老闆知識卡｜觀察篇",
    title: ["杯底有少量沉澱", "先別急著判斷"],
    bullets: ["先攪拌或搖勻", "觀察氣味與包裝", "有疑問再拍照詢問"],
    mascot: "faq",
  },
  "color": {
    eyebrow: "小老闆知識卡｜比較篇",
    title: ["顏色深淺", "能判斷品質嗎？"],
    bullets: ["不能只看顏色", "要看成分與規格", "保存與批次也要一起確認"],
    mascot: "choose",
  },
  "serving": {
    eyebrow: "小老闆知識卡｜標示篇",
    title: ["看總重量之外", "也要看每次份量"],
    bullets: ["總容量是整包規格", "每次取用量是日常安排", "兩個數字要分開理解"],
    mascot: "choose",
  },
  "one-format": {
    eyebrow: "小老闆知識卡｜日常篇",
    title: ["同一天不一定要", "安排多種型態"],
    bullets: ["依當天情境選一種即可", "外出、沖泡、料理分開想", "簡單才容易持續"],
    mascot: "choose",
  },
  "batch-info": {
    eyebrow: "小老闆知識卡｜保存篇",
    title: ["批號與保存資訊", "先留在哪裡？"],
    bullets: ["外盒先不要急著丟", "拍下批號與期限", "有問題比較容易確認"],
    mascot: "faq",
  },
  "support-photos": {
    eyebrow: "小老闆知識卡｜客服篇",
    title: ["詢問產品問題", "先拍這三張"],
    bullets: ["完整包裝", "批號與期限", "實際內容與保存狀況"],
    mascot: "faq",
  },
  "delivery-check": {
    eyebrow: "小老闆知識卡｜收貨篇",
    title: ["宅配收到後", "先做三個確認"],
    bullets: ["外箱有沒有破損", "產品規格是否正確", "依標示立即妥善保存"],
    mascot: "faq",
  },
  "clean-cup": {
    eyebrow: "小老闆知識卡｜沖泡篇",
    title: ["杯子殘留咖啡味", "會影響口感"],
    bullets: ["先把杯子洗乾淨", "避免其他飲品氣味", "再調整自己喜歡的濃淡"],
    mascot: "guide",
  },
  "soup-balance": {
    eyebrow: "小老闆知識卡｜料理篇",
    title: ["燉湯完成後", "再調整濃淡"],
    bullets: ["先看整鍋水量", "少量加入再試味道", "不用一次放得很複雜"],
    mascot: "guide",
  },
  "similar-name": {
    eyebrow: "小老闆知識卡｜選購篇",
    title: ["名稱相近的產品", "配方仍要逐項看"],
    bullets: ["先看完整成分", "再看規格與配料", "不要只看品名下判斷"],
    mascot: "choose",
  },
};
...
async function mascotBuffer(key) {
  ...
  const resized = await sharp(buffer).resize({ width: 470, height: 520, fit: "contain" }).png().toBuffer();
...
function svg(card) {
  ...
  const bulletMarkup = card.bullets.map((item, index) => `
    <g transform="translate(0 ${index * 92})">
      <circle cx="92" cy="783" r="25" fill="#315c45"/>
      <text x="92" y="792" text-anchor="middle" font-size="24" font-weight="800" fill="#fff">${index + 1}</text>
      <text x="140" y="793" font-size="34" font-weight="700" fill="#24211d">${esc(item)}</text>
    </g>`).join("");
  ...
  <svg width="1080" height="1350" viewBox="0 0 1080 1350" xmlns="http://www.w3.org/2000/svg">
    ...
    ${bulletMarkup}
    <rect x="575" y="670" width="418" height="500" rx="32" fill="#f2eadb" stroke="#d8c6a4" stroke-width="2"/>
    <text x="92" y="1195" font-family="Noto Sans TC, PingFang TC, sans-serif" font-size="27" font-weight="700" fill="#6b655d">看清楚內容，再依自己的日常選擇。</text>
...
async function renderCard(slug) {
  ...
  if (!card) return null;
  const base = sharp(Buffer.from(svg(card)));
  let output;
  try {
    const mascot = await mascotBuffer(card.mascot);
    output = await base.composite([{ input: mascot, left: 548, top: 672 }]).png({ quality: 94 }).toBuffer();
  } catch (error) {
    console.warn("knowledge card mascot fallback", slug, error.message);
    output = await base.png({ quality: 94 }).toBuffer();
  }
