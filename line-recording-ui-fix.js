...
const PRODUCTS = Object.freeze({
  "guilu-drink-30": { patterns: [/龜鹿飲\s*30\s*cc/i, /30\s*cc.*(?:玻璃罐|小玻璃罐)/i], image: `${SITE_BASE}images/products-v3/guilu-drink-30.jpg?v=${PRODUCT_IMAGE_VERSION}` },
  "guilu-drink-180": { patterns: [/龜鹿飲\s*180\s*cc/i, /180\s*cc.*鋁袋/i], image: `${SITE_BASE}images/products-v3/guilu-drink-180.jpg?v=${PRODUCT_IMAGE_VERSION}` },
  "guilu-gao": { patterns: [/龜鹿膏/], image: `${SITE_BASE}images/products-v3/guilu-gao.jpg?v=${PRODUCT_IMAGE_VERSION}` },
  "guilu-tangkuai": { patterns: [/龜鹿湯塊/], image: `${SITE_BASE}images/products-v3/guilu-tangkuai.jpg?v=${PRODUCT_IMAGE_VERSION}` },
  "guilu-jiao": { patterns: [/龜鹿膠/], image: `${SITE_BASE}images/products-v3/guilu-jiao.jpg?v=${PRODUCT_IMAGE_VERSION}` },
  "luerong-fen": { patterns: [/鹿茸粉/], image: `${SITE_BASE}images/products-v3/luerong-fen.jpg?v=${PRODUCT_IMAGE_VERSION}` },
});
...
function applyBubbleFix(bubble) {
  ...
  if (key) {
    ...
    if (oldAction && oldAction.type === "uri" && oldAction.uri && !/\.(?:jpe?g|png|webp)(?:[?#]|$)/i.test(oldAction.uri)) bubble.hero.action = oldAction;
    rewriteProductImageActions(bubble, key);
    bubble.xjwProductPhoto = key;
    bubble.xjwProductPhotoAuthority = "products-v3-latest-originals";
...
function pruneRepeatedMascotHeroes(carousel) {
  ...
  for (const bubble of carousel.contents) {
    if (!bubble || bubble.type !== "bubble" || bubble.xjwProductPhoto || !bubble.xjwRecommendationHero) continue;
...
module.exports = { VERSION, PRODUCT_IMAGE_VERSION, PRODUCTS, MASCOT_HEROES, GENERIC_MASCOT_HERO, RECOMMEND_HERO, MASCOT_CARD_PATTERN, rewriteVisibleText, normalizeVisibleText, collectTexts, uniqueProductKey, productHero, mascotSceneForText, mascotHero, rewriteProductImageActions, isRecommendationBubble, applyBubbleFix, pruneRepeatedMascotHeroes, applyVisualFix };
