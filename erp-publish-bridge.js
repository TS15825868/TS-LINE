...
async function publishInstagram(post) {
  ...
  const auth = await resolveInstagramAuth();
  const created = await graphPost(`${encodeURIComponent(auth.instagramUserId)}/media`, auth.token, {
    image_url: image,
    caption: postText(post),
  });
  ...
  for (let attempt = 0; attempt < 12; attempt += 1) {
    ...
    try {
      const status = await graphGet(creationId, auth.token, { fields: "status_code,status" });
  ...
  const published = await graphPost(`${encodeURIComponent(auth.instagramUserId)}/media_publish`, auth.token, {
    creation_id: creationId,
  });
  return { ...published, instagram_user_id: auth.instagramUserId, auth_source: auth.source };
}
