...
async function handleEvent(event){
  ...
  if(/套餐售價|套餐價格|搭配售價|搭配價格/.test(msg)) return reply(event.replyToken,textMsg(comboPriceReply(),mainQuick()));
  if(isOfferQuestion(msg)) return reply(event.replyToken,textMsg("目前會依搭配方式、數量與需求協助整理較適合的方案🙂\n你想先了解哪一項呢？",[{label:"單項活動",text:"單項售價"},{label:"套餐搭配",text:"套餐售價"},{label:"看產品",text:"看產品"}]));
  if(/龜鹿膠|湯塊跟膠|湯塊.*膠|膠.*湯塊|差別/.test(msg)) return reply(event.replyToken,textMsg("龜鹿湯塊與龜鹿膠使用相同原料與製程，內容物相同，差異在包裝方式與規格。\n\n龜鹿湯塊：75g（2兩／8塊，每塊約9.375g），精品小包裝，方便燉湯與沖泡。\n\n龜鹿膠：600g（一斤裝／32塊，每塊約18.75g），適合固定使用、家庭使用與通路合作。",mainQuick()));
  if(/配送|運費|寄送|親送|宅配/.test(msg)) return reply(event.replyToken,textMsg("【配送方式】\n\n✓ 宅配\n✓ 7-11賣貨便\n✓ 萬華門市自取\n✓ 雙北地區可安排親送\n\n詳細配送與運費由客服協助確認。",mainQuick()));
  if(/^(清空購買清單|清空清單|清空購物清單)$/.test(msg)){ state.cart=[]; state.checkout=null; return reply(event.replyToken,textMsg("購買清單已清空，可以重新挑選🙂",cartActions(state))); }
  ...
  if(msg.startsWith("刪除 ")||msg.startsWith("移除 ")){ const name=msg.replace(/^刪除\s*|^移除\s*/,"").trim(); const before=state.cart.length; state.cart=state.cart.filter(i=>i.name!==name); if(state.cart.length===before) return reply(event.replyToken,textMsg("購買清單裡沒有這個品項🙂",mainQuick())); return reply(event.replyToken,textMsg(`${name} 已移除。\n\n${cartText(state.cart)}`,cartActions(state))); }
  ...
  if(msg.startsWith("加入清單")||msg.startsWith("加入購買清單")){ const p=productByName(msg); const c=comboByName(msg); if(!p&&!c) return reply(event.replyToken,textMsg("找不到要加入的品項，請再點一次商品或套餐。",mainQuick())); const item=c?cartItemFromCombo(c):cartItemFromProduct(p); addToCart(state,item); return reply(event.replyToken,textMsg(`${item.name} 已加入購買清單🙂\n\n${cartText(state.cart)}`,cartActions(state))); }
  if(msg.startsWith("直接買")||msg.startsWith("我要買")||msg.startsWith("我要 ")){ const p=productByName(msg); const c=comboByName(msg); if(!p&&!c) return reply(event.replyToken,textMsg("可以先看產品或搭配組合，再點按鈕直接加入。",mainQuick())); const item=c?cartItemFromCombo(c):cartItemFromProduct(p); if(state.cart.length) return reply(event.replyToken,textMsg(`你目前購買清單裡已有品項。\n要把「${item.name}」加入清單，還是只買這個？`,[{label:"加入清單",text:`加入清單 ${item.name}`},{label:"只買這個",text:`只買 ${item.name}`},{label:"查看清單",text:"查看購買清單"}])); state.cart=[item]; return startCheckout(event,state); }
  if(msg.startsWith("只買 ")){ const p=productByName(msg); const c=comboByName(msg); if(!p&&!c) return reply(event.replyToken,textMsg("找不到這個品項，請再點一次。",mainQuick())); state.cart=[c?cartItemFromCombo(c):cartItemFromProduct(p)]; return startCheckout(event,state); }
  ...
  if(/推薦|怎麼選|適合哪個|不知道|Google|廣告|網站|食補|調養/.test(msg)) return reply(event.replyToken,textMsg("我先幫你用生活方式整理👇\n\n想固定節奏 → 龜鹿膏\n想方便快速 → 龜鹿飲\n想放進料理 → 龜鹿湯塊\n想固定使用或通路合作 → 龜鹿膠\n想自己搭配 → 鹿茸粉",[{label:"看產品",text:"看產品"},{label:"單項售價",text:"單項售價"},{label:"套餐售價",text:"套餐售價"}]));
...
async function continueCheckout(event,state,msg){
  ...
  if(ck.step==="phone"){ ck.phone=msg; ck.step="address"; return reply(event.replyToken,textMsg("收到。請回覆收件地址、7-11門市資訊，或雙北親送地址。",[{label:"取消",text:"取消"}])); }
  ...
  if(ck.step==="shipping"){ ck.shipping=msg.replace(/^配送\s*/,""); ck.step="confirm"; return reply(event.replyToken,textMsg(`請確認訂單👇\n\n${cartText(state.cart)}\n\n姓名：${ck.name}\n電話：${ck.phone}\n地址／門市：${ck.address}\n付款：${ck.payment}\n配送：${ck.shipping}`,[{label:"確認送出",text:"確認送出"},{label:"修改清單",text:"查看購買清單"},{label:"取消",text:"取消"}])); }
  if(ck.step==="confirm" && msg==="確認送出"){ const summary={cart:state.cart,total:cartTotal(state.cart),...ck,createdAt:new Date().toISOString()}; await saveCRM(summary); state.cart=[]; state.checkout=null; return reply(event.replyToken,textMsg("已收到你的資料，我們會再為你確認金額、活動與配送安排🙂",mainQuick())); }
  return reply(event.replyToken,textMsg("請依照按鈕或提示回覆。",[{label:"確認送出",text:"確認送出"},{label:"取消",text:"取消"}]));
}
async function saveCRM(data){ if(!CRM_URL) return; try{ await fetch(CRM_URL,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(data)}); }catch(e){ console.error("CRM error",e); } }
