...
function qtyMenuFlex(product, mode = "add") {
  const buttons = [
    { label: `1${product.unit}｜${money(product.price)}`, data: pb("set_qty", { productId: product.id, qty: "1", mode }) },
...
function recommendResultFlex(keyword) {
  ...
  return flexCard(p ? `推薦：${p.displayName || p.name}` : "推薦商品", desc, [
...
async function handlePostback(event) {
  ...
  if (p.action === "set_qty") {
    ...
    if (p.mode === "replace") {
      addProductToCart(state, product, p.qty, true);
      return reply(event.replyToken, cartFlex(state));
    }
    addProductToCart(state, product, p.qty, false);
    return reply(event.replyToken, cartFlex(state));
  }
