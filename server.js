...
function productById(text) {
  const match = String(text || "").match(/#([a-zA-Z0-9-_]+)/);
  if (!match) return null;
  return DATA.products.find((p) => p.id === match[1]) || null;
}

function productByName(text) {
  const byId = productById(text);
  if (byId) return byId;

  const raw = cleanName(text);

  return DATA.products.find((p) => {
    return p.name === raw || p.displayName === raw || (p.aliases || []).includes(raw);
  });
}

function comboById(text) {
  const match = String(text || "").match(/#([a-zA-Z0-9-_]+)/);
  if (!match) return null;
  return DATA.combos.find((c) => c.id === match[1]) || null;
}

function comboByName(text) {
  const byId = comboById(text);
  if (byId) return byId;

  const raw = cleanName(text);

  return DATA.combos.find((c) => {
    return c.name === raw || String(text).includes(c.name) || (c.aliases || []).includes(raw);
  });
}
...
function cartFlex(state) {
  ...
  if (!cart.length) {
    return textMsg("目前購買清單是空的。", mainQuick());
  }

  const itemText = cart
    .map((i, idx) => {
      const subtotal = (i.price || 0) * (i.qty || 1);
      return `${idx + 1}. ${i.name} × ${i.qty}\n${money(subtotal)}`;
    })
    .join("\n\n");
    ...
    contents: {
      ...
      body: {
        ...
        contents: [
          ...
          {
            type: "text",
            text: `預估合計：${money(cartTotal(cart))}`,
            weight: "bold",
            size: "md",
            color: "#111111",
          },
        ...
        contents: [
          {
            ...
            style: "primary",
            color: "#7B1E1E",
            action: { type: "message", label: "繼續選購", text: "看產品" },
          },
          {
            type: "button",
            style: "secondary",
            action: { type: "message", label: "移除商品", text: "移除商品" },
          },
          {
            type: "button",
            style: "secondary",
            action: { type: "message", label: "清空清單", text: "清空購買清單" },
          },
          {
            type: "button",
            style: "secondary",
            action: { type: "message", label: "直接結帳", text: "直接結帳" },
          },
        ],
      },
    },
...
function usageMenuFlex() {
  return checkoutCard("仙加味｜怎麼使用", "請選擇想了解的產品，我們會整理對應的使用方式。", [
    { label: "龜鹿膏", text: "食用方式#guilu-gao" },
    { label: "龜鹿湯塊", text: "食用方式#guilu-tangkuai" },
    { label: "龜鹿膠", text: "食用方式#guilu-jiao" },
    { label: "龜鹿飲", text: "食用方式#guilu-drink-30" },
    { label: "鹿茸粉", text: "食用方式#luerong-fen" },
  ]);
}

function recommendFlex() {
  return checkoutCard("仙加味｜產品建議", "依平常使用習慣選擇，先從最容易持續的方式開始。", [
    { label: "固定補養", text: "推薦 固定補養" },
    { label: "方便飲用", text: "推薦 方便飲用" },
    { label: "料理燉湯", text: "推薦 料理燉湯" },
    { label: "固定使用", text: "推薦 固定使用" },
    { label: "自行搭配", text: "推薦 自行搭配" },
  ]);
}
...
async function handleEvent(event) {
  if (event.type === "follow") {
    return reply(event.replyToken, textMsg("歡迎來到仙加味・龜鹿🙂\n可以直接點下面按鈕，不用自己打字。", mainQuick()));
  }
  ...
  if (msg.startsWith("建議售價") || msg.startsWith("價格方案#")) {
    const p = productByName(msg);
    return reply(event.replyToken, textMsg(p ? productPriceText(p) : "請先點產品卡上的價格方案按鈕。", mainQuick()));
  }

  if (msg.startsWith("活動優惠")) {
    const p = productByName(msg);
    if (p) return reply(event.replyToken, textMsg(productActivityText(p), mainQuick()));
    return reply(event.replyToken, textMsg(activityReply(), mainQuick()));
  }

  if (msg.startsWith("食用方式")) {
    const p = productByName(msg);
    return reply(event.replyToken, textMsg(p ? productUsageText(p) : "請先選擇產品。", mainQuick()));
  }

  if (msg === "價格方案" || /價格|售價|價錢|多少錢/.test(msg)) {
    return reply(event.replyToken, priceMenuFlex());
  }

  if (/單項售價|單品售價|單品價格|單項價格/.test(msg)) {
    return reply(event.replyToken, textMsg(singlePriceReply(), mainQuick()));
  }

  if (/套餐售價|套餐價格|套餐搭配|搭配售價|搭配價格/.test(msg)) {
    return reply(event.replyToken, [textMsg("以下是目前常見搭配方向："), comboCarousel()]);
  }

  if (/活動|優惠|折扣|比較便宜|便宜一點|買多|方案|有比較划算|有沒有優惠|能不能便宜/.test(msg)) {
    return reply(event.replyToken, textMsg(activityReply(), mainQuick()));
  }

  if (/怎麼使用|使用方式|怎麼吃|食用方式/.test(msg)) {
    return reply(event.replyToken, usageMenuFlex());
  }

  if (/幫我推薦|推薦|怎麼選|適合哪個|不知道/.test(msg)) {
    return reply(event.replyToken, recommendFlex());
  }

  if (msg.startsWith("推薦 ")) {
    return reply(event.replyToken, textMsg(recommendationText(msg), mainQuick()));
  }
  ...
  if (/配送|運費|寄送|親送|宅配/.test(msg)) {
    return reply(
      event.replyToken,
      checkoutCard("配送方式", "請選擇你方便的配送方式。詳細配送與運費會由客服協助確認。", [
        { label: "宅配", text: "宅配" },
...
function startCheckout(event, state) {
  ...
  return reply(event.replyToken, [
    cartFlex(state),
    checkoutCard("第一步｜收件姓名", "請直接回覆收件人姓名。", [{ label: "取消", text: "取消" }]),
  ]);
}
