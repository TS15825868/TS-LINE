 ...
 * - 成分可完整回覆
 *   - 龜鹿湯塊（膠）成分：只保留「鹿角、全龜」（依你要求）
 * - 價格採「單品回覆」
 *   - 建議售價 + 目前活動價（售價9折）
 *   - 加上「不同通路略有差異」「到店活動依現場為準」
 * - 購買方式採「先選方式 → 再問品項/數量/聯絡資訊」，不制式填單、不容易卡住
 * - 真人回覆管理（真人模式中暫停自動，避免打架；可解除）
 * - 雙北親送：加地址判斷（台北/新北優先提示可協助安排；若不便親送改宅配/店到店）
 */
...
const STORE = {
  ...
  products: {
    ...
    drink: {
      ...
      faq: [
        { q: "可以冰的喝嗎？", a: "建議溫熱飲用較為合適；若個人習慣冷飲，建議先少量嘗試。" },
      ],
    },
    ...
    antler: {
      ...
      faq: [
        { q: "可以加在咖啡/牛奶嗎？", a: "建議以溫熱飲品為主；若加在飲品中，先少量嘗試口感。" },
      ],
    },
...
function productIntroText(userId, key) {
  ...
  if (key === "soup") {
    const vLines = STORE.products.soup.variants.map(v => {
...
function productPriceText(userId, key) {
  ...
  if (key === "soup") {
    ...
    for (const v of STORE.products.soup.variants) {
      ...
      if (act) lines.push(`目前活動價：${money(act)}（售價9折）`);
...
function soupVariantPriceText(code) {
  ...
  if (act) out.push(`目前活動價：${money(act)}（售價9折）`);
...
async function handleEvent(event) {
  ...
  if (lastMenu === "product_menu" && ["1","2","3","4"].includes(rawNorm)) {
    ...
    if (key === "soup") {
      updateUser(userId, (u) => { u.state.lastMenu = "soup_variant_menu"; });
  ...
  if (lastMenu === "soup_variant_menu" && ["1","2","3","4"].includes(rawNorm)) {
    updateUser(userId, (u) => { u.state.lastMenu = "soup_variant_menu"; u.state.lastProductKey = "soup"; });
  ...
  if (lastMenu === "price_menu" && ["1","2","3","4"].includes(rawNorm)) {
    const map = { "1": "gel", "2": "drink", "3": "antler", "4": "soup" };
    const key = map[rawNorm];
    updateUser(userId, (u) => { u.state.lastProductKey = key; u.state.lastMenu = "price_menu"; });
  ...
  if (flags.ingredients) {
    ...
    updateUser(userId, (u) => { u.state.lastMenu = menu; u.state.lastProductKey = key; });
