...
/* =========================
   products.json 快取
========================= */
// products.json 快取（強化：即便抓取失敗也會回傳「上一份成功快取」，避免 webhook 卡住）
let cache = { at: 0, lastOkAt: 0, data: null, lastError: null };
...
async function getProducts() {
  ...
  try {
    ...
    cache = { at: Date.now(), lastOkAt: Date.now(), data, lastError: null };
    return data;
  } catch (e) {
    cache.lastError = e?.message || String(e);
    console.error("[products] 讀取失敗（改用快取）：", cache.lastError);
    // 強化：若曾成功抓到資料 → 退回舊快取（就算超過 ttl）
    if (cache.data) return cache.data;
    return { categories: [] };
  }
}

// 強化：啟動後定時預抓（降低首次查詢延遲，減少 webhook timeout 機率）
async function prewarmProducts() {
  try {
    await getProducts();
  } catch {
    // ignore
...
prewarmProducts();
setInterval(prewarmProducts, 2 * 60 * 1000).unref?.();
...
// Promise 超時保護（避免偶發網路/LINE API 卡住拖垮 webhook）
...
async function safeReply(replyToken, msgs) {
  try {
    await withTimeout(client.replyMessage(replyToken, msgs), 8000, "reply timeout");
  } catch (e) {
    console.error("[reply] 送出失敗：", e?.message || e);
  }
}
  ...
  return [
    {
      type: "text",
      text:
        "歡迎加入 仙加味・龜鹿 🙂\n\n想先看產品、價格、或飲食搭配建議，直接回『選單』或點下方選單即可。\n（本帳號提供食品資訊/搭配參考，不提供醫療諮詢）",
    },
    ...mainMenuCards(),
  ];
}
...
async function handleText(userId, text) {
  ...
  // 避免資訊還沒看完就被「主選單」洗版：主選單只在使用者輸入「選單」時送出
...
app.post(
  ...
  async (req, res) => {
    // ✅ 先回 200，避免 LINE Developers「Webhook verify」偶發 timeout
    // 後續 replyMessage 是另外一支 API，不依賴此 request 的回應速度
    res.sendStatus(200);
      ...
      events.map(async (event) => {
        try {
          ...
          if (event.type === "follow") {
            await safeReply(event.replyToken, welcomeMessages());
            return;
          }
          if (event.type === "join") {
            await safeReply(event.replyToken, welcomeMessages());
          ...
          const msgs = await withTimeout(
            handleText(userId, event.message.text),
            8500,
            "handleText timeout"
          );
          await safeReply(event.replyToken, msgs);
        } catch (e) {
          console.error("[webhook] event error:", e?.message || e);
        }
      })
    );
  }
