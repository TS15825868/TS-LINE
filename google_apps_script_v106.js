function doPost(e) {
  try {
    ...
    var cart = data.cart || [];
    var productText = data.productText || cart.map(function(i) {
      var qty = (i.qty || 1) + (i.unit || "");
      var label = i.label ? "（" + i.label + "）" : "";
      var subtotal = i.total ? " 小計" + i.total : "";
      return i.name + " × " + qty + label + subtotal;
    }).join("、");

    var qtyText = cart.map(function(i) {
      return (i.qty || 1) + (i.unit || "");
    }).join("、");
