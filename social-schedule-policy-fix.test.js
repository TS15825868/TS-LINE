...
const socialServer = fs.readFileSync(path.join(__dirname, "social-server.js"), "utf8");
const rebuiltServer = fix.patchSource(path.join(__dirname, "social-server.js"), socialServer);
...
const rebuild = fs.readFileSync(path.join(__dirname, "social-official-rebuild.js"), "utf8");
const rebuiltSchedule = fix.patchSource(path.join(__dirname, "social-official-rebuild.js"), rebuild);
...
assert.equal(fix.VERSION, "20260808-tue1930-sat0930-v1");

console.log("PASS：LINE 社群固定排程已鎖週二19:30／週六09:30，舊未發布草稿啟動時重新對齊；立即發布不受固定時段限制。");
