...
const RETIRED_COPY_REPLACEMENTS = Object.freeze([
  ...
  [/每日\s*1\s*[-–～至]\s*2\s*罐/g, CURRENT_30_USAGE],
...
function sanitizeCurrentCopy(value) {
  ...
  if (value && typeof value === "object") {
    return Object.fromEntries(Object.entries(value).map(([key, item]) => [key, sanitizeCurrentCopy(item)]));
  }
...
function salesOverride(override = {}) {
  return Object.fromEntries(
    SALES_OVERRIDE_FIELDS
      ...
      .map((field) => [field, sanitizeCurrentCopy(override[field])])
  );
}
...
function currentAuthorityOverride(id, merged = {}) {
  ...
  const aliases = (Array.isArray(merged.aliases) ? merged.aliases : [])
    .map((v) => sanitizeCurrentCopy(String(v || "").trim()))
    .filter(Boolean)
    .filter((v) => id !== "guilu-drink-30" || !/瓶/.test(v))
    .filter((v) => id !== "guilu-jiao" || !/^一斤$/.test(v));
...
function photoOverride(id) {
  ...
  const official = authorityProduct(id);
  const productImage = String(official?.approvedProductImage || "").trim();
  const detailedDm = String(official?.approvedDm || "").trim();
  if (!original) throw new Error(`${id} 缺少 products-v3 正式產品原圖權威`);
  const customerProductImage = productImage || original;
  const customerDm = detailedDm || customerProductImage;
  return {
    image: customerProductImage,
    imageUrl: customerProductImage,
...
function applyMaster(data) {
  ...
  data.salesMasterSource = `${policy.source}; product image / detailed DM / trial roles separated by current official authority`;
