...
(() => {
  const VERSION = "20260716-mobile-1";
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      @media(max-width:760px){
        ...
        body{padding-bottom:calc(92px + env(safe-area-inset-bottom))!important;overflow-x:hidden}
        ...
        .xjw-safe-online{right:10px!important;bottom:calc(92px + env(safe-area-inset-bottom))!important;max-width:190px!important;font-size:11px!important;pointer-events:none!important;transition:opacity .25s ease,transform .25s ease!important}
  ...
  function syncActive() {
    ...
    const more = byId("xjwMobileMore");
    const primaryViews = new Set(PRIMARY.map((item) => item.view));
    more?.classList.toggle("active", !primaryViews.has(active));
  }

  function buildNavigation() {
    if (byId("xjwMobileNav")) {
      buildMoreItems();
      syncActive();
      return;
    }
    ...
    nav.innerHTML = PRIMARY.map((item) => `
      <button type="button" data-mobile-view="${item.view}" aria-label="${item.label}">
        <span class="xjw-mobile-icon" aria-hidden="true">${item.icon}</span><span>${item.label}</span>
      </button>`).join("") + `
      <button id="xjwMobileMore" type="button" aria-expanded="false" aria-controls="xjwMobileSheet">
        <span class="xjw-mobile-icon" aria-hidden="true">•••</span><span>更多</span>
      </button>`;
  ...
  function tameConnectionBadge() {
    ...
    const fade = () => {
      ...
      else badge.classList.remove("xjw-mobile-hidden");
    };
    window.addEventListener("online", () => { badge.classList.remove("xjw-mobile-hidden"); fade(); });
