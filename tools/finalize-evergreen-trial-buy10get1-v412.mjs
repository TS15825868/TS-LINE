...
const LEAD_TIME_DEFINITION = '5～7個工作天只計製作加工，不含例假日及完成後的物流配送時間';
const textFiles = ['server.js', 'test.js', 'function.test.js', 'catalog.test.js', 'security.test.js', 'README.md'];
const replacements = [
  ...
  ['訂單資料與付款方式確認完成後，約5～7個工作天安排出貨，不含例假日及物流時間', `訂單資料與付款方式確認完成後，${OFFICIAL_FULFILLMENT}`],
...
data.trialCampaign = {
  ...
  leadTime: '製作加工約需5～7個工作天',
  leadTimeDefinition: LEAD_TIME_DEFINITION,
  shippingRule: '製作加工完成後才安排出貨',
...
data.shippingNotes = {
  ...(data.shippingNotes || {}),
  宅配: `訂單資料與付款方式確認完成後，${OFFICIAL_FULFILLMENT}。試喝組郵局宅配運費100元。`,
  '7-11賣貨便': `訂單資料與付款方式確認完成後，${OFFICIAL_FULFILLMENT}。試喝組7-11店到店運費60元。`,
};
...
if (existsSync('line-sales-master.json')) {
  ...
  if (m30) Object.assign(m30, {
    name: '龜鹿飲30cc玻璃罐', displayName: '龜鹿飲30cc玻璃罐', specification: '30cc／罐（小玻璃罐）', size: '30cc／罐（小玻璃罐）', spec: '30cc／罐（小玻璃罐）', unit: '罐', price: 50,
    offers: ['買10送1'], quantityOptions: [1, 3, 5, 11], offer: { qty: 11, total: 500, label: '買10送1' },
    priceText: '$50 / 罐', priceLabel: '售價50元，買10送1（共11罐500元）',
  });
  if (m180) Object.assign(m180, {
    name: '龜鹿飲180cc鋁袋', displayName: '龜鹿飲180cc鋁袋', specification: '180cc／包（鋁袋）', size: '180cc／包（鋁袋）', spec: '180cc／包（鋁袋）', unit: '包', price: 200,
    offers: ['買10送1'], quantityOptions: [1, 3, 5, 11], offer: { qty: 11, total: 2000, label: '買10送1' },
    priceText: '$200 / 包', priceLabel: '售價200元，買10送1（共11包2,000元）',
  });
  master.trialCampaign = data.trialCampaign;
  master.shippingNotes = data.shippingNotes;
...
if (existsSync('package.json')) {
  ...
  for (const key of Object.keys(pkg.scripts || {})) {
    pkg.scripts[key] = String(pkg.scripts[key])
      .replace(/\s*&&\s*node tools\/finalize-trial-production-lead-time-v415\.mjs/g, '')
      .replace(/\s*&&\s*node tools\/finalize-trial-shipping-v416\.mjs/g, '');
  }
  pkg.version = '6.1.5';
  pkg.description = '仙加味 LINE OA 正式版｜30cc三罐試喝品免費、運費自付｜30cc與180cc買10送1｜製作加工約需5～7個工作天，完成後出貨｜人工審核後發布';
...
if (!verification.includes('買10送1') || !verification.includes('製作加工約需5～7個工作天') || !verification.includes('完成後才安排出貨')) {
  throw new Error('LINE母本缺少正式活動或製作加工／出貨規則');
}
console.log('PASS LINE OA：30cc／180cc買10送1；5～7個工作天只計製作加工，完成後才安排出貨。');
