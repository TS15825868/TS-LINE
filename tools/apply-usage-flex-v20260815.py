...
def clean_usage(pid,usage):
 ...
 retired=(OLD_GAO,'一天一次一小匙','早晚各一小匙',OLD_DAY,'每日一罐','每日1罐','每日 1 罐','每日一包','避免接近睡前')
...
rules=[str(x) for x in auth.get('guardRules',[]) if OLD_GAO not in str(x) and OLD_DAY not in str(x) and '一天一次' not in str(x) and '早晚各一小匙' not in str(x) and '每日一罐' not in str(x) and '每日一包' not in str(x)]
rules.extend([
...
repls=[(OLD_GAO,GAO),('建議早上與下午各一小匙',GAO),('早上與下午各一小匙',GAO),(OLD_DAY,DRINK_TIME),('每日一罐',DRINK30),('每日 1 罐',DRINK30),('每日1罐',DRINK30),('每日一包',DRINK180),('避免接近睡前食用',GAO),('避免接近睡前',GAO)]
...
assert OLD_GAO not in blob and OLD_DAY not in blob and '每日一罐' not in blob and '每日一包' not in blob
