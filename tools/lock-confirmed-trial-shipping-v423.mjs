...
const OFFICIAL_FULFILLMENT = '資料及運費確認完成後採接單安排製作，約5～7個工作天出貨，不含例假日及物流配送時間';
const ORDER_FULFILLMENT = '訂單資料與付款方式確認完成後採接單安排製作，約5～7個工作天出貨，不含例假日及物流配送時間';
const LEGACY_PHRASES = [
  '買10送2', '共12罐500元', '共12包2,000元',
  '30cc／瓶（小玻璃瓶）', '龜鹿飲30cc玻璃瓶', '小玻璃瓶外出攜帶',
  '製作加工約需5～7個工作天', '5～7個工作天只計製作加工', '5～7個工作天僅指製作加工'
];
...
const replacements = [
  ...
  ['資料及運費確認完成後安排製作加工，製作加工約需5～7個工作天；完成後才安排出貨，物流配送時間另計', OFFICIAL_FULFILLMENT],
  ['資料與運費確認完成後安排製作加工，製作加工約需5～7個工作天；完成後才安排出貨，物流配送時間另計', OFFICIAL_FULFILLMENT],
...
for (const file of [
  ...
  'tools/apply-evergreen-trial-campaign.mjs', 'tools/repair-evergreen-trial-tests.mjs',
  'line-order-cart.js', 'line-order-cart.test.js', 'internal-inventory-seed.js', 'internal-inventory-seed.test.js',
  'internal-order-pricing.js', 'internal-order-pricing.test.js', 'internal-line-order-sync.js', 'internal-line-order-sync.test.js'
]) {
  if (!existsSync(file)) continue;
  writeFileSync(file, normalizeText(readFileSync(file, 'utf8')));
}
...
for (const [file, object] of [['data.json', data], ['line-sales-master.json', master]]) {
  ...
  for (const bad of LEGACY_PHRASES) if (text.includes(bad)) throw new Error(`${file} 仍含舊資料：${bad}`);
...
writeFileSync('diagnostics/user-confirmed-trial-v426.json', JSON.stringify({
  status: 'success', version: 'v426', checked_at: new Date().toISOString(),
  products: ['龜鹿飲30cc玻璃罐｜30cc／罐（小玻璃罐）', '龜鹿飲180cc鋁袋｜180cc／包（鋁袋）'],
  trial: '30cc小玻璃罐3罐免費；7-11店到店60元；郵局宅配100元；每位顧客／電話／地址限一次；LINE OA完成',
  fulfillment: '接單後安排製作，約5～7個工作天出貨，不含例假日及物流配送時間',
  promotions: ['30cc買10送1，共11罐500元', '180cc買10送1，共11包2,000元'],
  owner_review_required: true
}, null, 2) + '\n');
console.log('PASS LINE OA v426：小玻璃罐、3罐免費試喝、買10送1、約5～7個工作天出貨與LINE OA導流均已鎖定。');
