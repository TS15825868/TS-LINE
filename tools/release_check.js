...
if (data.version !== "401.4") errors.push("data.json 版本未同步至 401.4");
if (data.catalogVersion !== "408.7") errors.push("官網素材版本未同步至 408.7");
if (data.lineBotVersion !== "v401.4") errors.push("LINE OA 版本未同步至 v401.4");
if (data.lineAssetsVersion !== "401.4") errors.push("小老闆素材版本未同步至 401.4");
if (data.runtime?.version !== "401.4") errors.push("中央 runtime 設定不正確");
...
if (!server.includes('const VERSION = "v401.4";')) errors.push("server.js 版本不正確");
if (!server.includes('const MASCOT_VERSION = "401.4";')) errors.push("小老闆快取版本不正確");
if (pkg.version !== "4.1.4" || pkg.scripts?.start !== "node server.js") errors.push("package.json 未整合為單一主程式");
...
console.log(`LINE OA v401.4 整合檢查通過：單一 server.js、中央 data.json、${data.products.length} 項產品、Webhook、購物車、結帳、CRM 與圖片政策正常。`);
