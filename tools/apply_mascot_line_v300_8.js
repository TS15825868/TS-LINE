...
functionTest = replaceOnce(functionTest, "assert.strictEqual(recommendReply().contents.contents.length, 3);", "assert.strictEqual(recommendReply().contents.contents.length, 4);", "推薦卡數量測試");
functionTest = replaceOnce(functionTest, "assert.strictEqual(comboMenuReply().contents.contents.length, DATA.offers.comboOffers.length);", "assert.strictEqual(comboMenuReply().contents.contents.length, DATA.offers.comboOffers.length + 1);", "搭配卡數量測試");
functionTest = replaceOnce(functionTest, "assert.strictEqual(usageChooserReply().contents.contents.length, 6);", "assert.strictEqual(usageChooserReply().contents.contents.length, DATA.products.length + 1);", "使用卡數量測試");
