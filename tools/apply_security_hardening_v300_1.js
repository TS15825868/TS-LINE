...
function pb`, "狀態管理");
server = replaceRegex(server, /async function saveCRM\(payload\) \{[\s\S]*?\n\}\n\nasync function continueCheckout/, `async function saveCRM(payload) {
...
async function continueCheckout`, "CRM 逾時");
...
let test = read("test.js").replace('assert.strictEqual(VERSION, "v300.0");', 'assert.strictEqual(VERSION, "v300.1");');
write("test.js", test);
