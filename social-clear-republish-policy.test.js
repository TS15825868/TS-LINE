...
(async () => {
  ...
  assert.strictEqual(post.sourceImageFile, "care-work-rest-clear-vector-1254.svg");
  assert.strictEqual(post.referenceImageFile, "D558F584-DEE1-45BB-B243-6166E118617C.PNG");
  ...
  assert.strictEqual(info.ok, true, "清晰向量圖無法正常輸出");
  assert(info.width >= 1200 && info.height >= 1200, `清晰圖尺寸不足：${info.width}×${info.height}`);
  assert(info.bytes >= 400000, `清晰圖壓縮過度：${info.bytes} bytes`);
  ...
  console.log(`PASS clear vector republish ${post.id} ${info.width}x${info.height} ${info.bytes} bytes`);
})().catch((error) => {
  console.error(error);
  process.exit(1);
});
