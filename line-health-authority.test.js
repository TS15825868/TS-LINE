...
assert.equal(payload.guiluTangkuaiSpecification, "75g／盒｜8塊裝｜每塊約9.375g");
assert.equal(payload.guiluJiaoSpecification, "600g（1斤）／盒｜32塊裝｜每塊約18.75g");
assert.equal(payload.luerongFenSpecification, "75g／罐");
for (const forbidden of ["publishingReview", "lineVoom", "schedulePolicy", "products-v2-actual-photos", "marketing-layout-reference-only"]) {
  assert.ok(!JSON.stringify(payload).includes(forbidden), `LINE健康診斷不得保留舊／非LINE正式欄位：${forbidden}`);
}
...
console.log("PASS：LINE健康診斷只描述獨立LINE OA正式狀態，products-v3、個別產品規格與尺寸規則正確，無舊products-v2／貼文／社群排程欄位。");
