...
assert.strictEqual(VERSION, "v401.5");
const messages = [
...
console.log("PASS full LINE function matrix v401.5");
...
console.log("PASS quantity options and promotions v401.5");
...
console.log("PASS website message routing v401.5");
...
console.log("PASS combo prices, quantities, promotions and cart v401.5");
...
console.log("PASS LINE product images and final DM buttons v401.5");
...
for (const message of [mascotWelcomeReply(), recommendReply(), usageChooserReply(), faqReply()]) {
  ...
  assert.ok(bubble.hero.url.includes("/TS-LINE/main/public/mascot/"), "小老闆卡未使用 TS-LINE 本地圖片：" + bubble.hero.url);
  assert.strictEqual(bubble.hero.aspectMode, "fit");
}
console.log("PASS LINE OA dedicated local mascot cards v401.5");
