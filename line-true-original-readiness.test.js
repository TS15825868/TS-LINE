...
const CACHE = "20260810-products-v3-latest-originals-v3";
...
for (const [id, url] of Object.entries(authority.products || {})) {
  ...
  assert.ok(String(url).includes(CACHE), `${id} 未使用最新產品原圖快取版本`);
}
...
for (const product of data.products) {
  for (const field of ["image", "imageUrl", "image_url", "dmImage", "officialOriginalImage"]) {
    ...
    assert.ok(url.includes(CACHE), `${product.id}.${field} 未使用最新產品原圖快取版本`);
...
for (const [id, item] of Object.entries(visual.PRODUCTS || {})) {
  ...
  assert.ok(String(item.image || "").includes(CACHE), `${id} Flex hero 未使用最新產品原圖快取版本`);
}

console.log("PASS：Render prestart 已鎖定六項 products-v3 最新真正產品實拍與 2026-08-10 v3 快取版本，LINE Flex 與銷售母本不得回退舊圖／舊海報圖。");
